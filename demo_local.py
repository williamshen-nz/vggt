...
def demo():
    ...
    polycam_dataset = (
        "/home/wilshen/datasets/feijoa/2025-05-18_the-one-that-couldnt-get-away"
    )
    ...
    predictions = vggt_inference(model, image_paths)
    device = "cuda"
    ...
    rr.log(
        "scaled_pcd",
        rr.Points3D(
            positions=down_pts.cpu(), colors=down_rgbs.cpu(), radii=voxel_size / 2
        ),
    )
    ...
    for src_path, rgb, depth, c2w, intrinsic in zip(
        image_paths, rgbs, scaled_depth_maps, vggt_aligned_c2ws, intrinsics
    ):
...
if __name__ == "__main__":
    # TODO:
    #  1. Server takes in image paths
    #  2. Server outputs depths, transforms and intrinsics
    #  3. Client takes transforms and computes R, t, s using Umeyama
    #  4. Applies this locally and loads things
    demo()
